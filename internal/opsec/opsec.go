...
var ModuleNoise = map[string]ModuleInfo{
	...
	"catcher":        {NoiseLow, "passive TCP listener — no outbound traffic"},
	"looter":         {NoiseMedium, "SSH session with enumeration commands"},
	"cred_tester":    {NoiseHigh, "multiple auth attempts — risk of account lockout"},
	"bruteforce":     {NoiseNone, "local hash cracking — no network traffic"},
