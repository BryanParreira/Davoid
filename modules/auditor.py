import os  # CRITICAL: Missing from previous version
import shutil
import subprocess
import platform
...
class DavoidAuditor:
    def check_and_fix(self, dep):
        ...
        if not shutil.which(dep):
            ...
            if platform.system() == "Darwin":  # macOS
                if dep == "airmon-ng": console.print("    [dim]Remedy: brew install aircrack-ng[/dim]")
                elif dep == "iw": console.print("    [dim]Note: 'iw' is Linux-only. Use 'airport' on Mac.[/dim]")
                else: console.print(f"    [dim]Remedy: brew install {dep}[/dim]")
            else:  # Linux
    ...
    def run(self):
        ...
        if platform.system() == "Darwin": deps.append("airport")
        else: deps.append("iw")
        ...
        if iface:
            try:
                if platform.system() == "Darwin":
                    subprocess.run(["sudo", "airport", iface, "disassociate"], capture_output=True)
                    console.print(f"[bold green][+] {iface} disassociated for sniffing.[/bold green]")
                else:
                    subprocess.run(["sudo", "airmon-ng", "start", iface], capture_output=True)
                    console.print(f"[bold green][+] {iface} set to Monitor Mode.[/bold green]")
            except Exception as e:
                console.print(f"[bold red][!] Could not set Monitor Mode: {e}[/bold red]")
