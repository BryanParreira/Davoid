...
class DNSReconEngine:
    ...
    def get_passive_creds(self):
        ...
        try:
            ...
            console.print(
                f"[dim red][!] Passive discovery failed (CRT.sh): {e}[/dim red]")
        return []
    ...
    def run(self):
        ...
        console.print(Panel(
            f"Targeting Domain: [bold cyan]{self.domain}[/bold cyan]", border_style="magenta"))

        # 1. Standard Infrastructure Records
        record_table = Table(title="Core Infrastructure Records",
        ...
        with Progress(SpinnerColumn(), TextColumn("{task.description}"), console=console) as progress:
            # Passive Stage
            task_passive = progress.add_task(
                "[yellow]Collecting Passive OSINT (CT Logs)...", total=None)
            passive_subs = self.get_passive_creds()
            progress.update(
                task_passive, description="[green]Passive OSINT Collection Complete.")

            # Active Brute Force Stage
            task_active = progress.add_task(
                "[cyan]Active Brute-Force Discovery...", total=len(SUB_DICT))
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(self.brute_worker, s)
                           for s in SUB_DICT]
                for _ in as_completed(futures):
                    progress.update(task_active, advance=1)

            # Verification Stage (Passive to Active)
            task_verify = progress.add_task(
        ...
        if self.found_assets:
            sub_table = Table(
                title=f"Infrastructure Map: {len(self.found_assets)} Assets Found", border_style="green", expand=True)
            ...
            # Export Option
            if questionary.confirm("Export results to JSON?", default=False, style=Q_STYLE).ask():
