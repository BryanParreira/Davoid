...
class ScannerEngine:
    ...
    def network_discovery(self):
        ...
        try:
            ...
            console.print(Panel(f"Interface: [bold cyan]{conf.iface}[/bold cyan] | Local IP: [bold cyan]{local_ip}[/bold cyan]\nDefault Subnet: [bold cyan]{subnet_hint}[/bold cyan]", title="Network Context", border_style="dim"))
        except:
            subnet_hint = "192.168.1.0/24"

        target = console.input(f"[bold yellow]Scan Range (Default {subnet_hint}): [/bold yellow]").strip() or subnet_hint
        do_deep = console.input("[bold cyan]Enable Deep Fingerprinting (-sV)? (y/N): [/bold cyan]").lower() == 'y'
        ...
        with Progress(
            ...
            task1 = progress.add_task("[cyan]L2 ARP Mapping (Subnet Discovery)...", total=None)
            ...
            progress.update(task1, completed=100, description="[cyan]ARP Discovery Finished.")

            # Phase 2: Stealth SYN Sweep for hidden hosts
            ip_list = [str(ip) for ip in IPNetwork(target)]
            task2 = progress.add_task("[yellow]Stealth Scanning (SYN Sweep)...", total=len(ip_list))
            ...
            with ThreadPoolExecutor(max_workers=50) as executor:
                ...
                for f in as_completed(futures):
                    ...
                    if f.result() and not any(h["ip"] == ip for h in active_hosts):
                        ...
                        res, _ = srp(Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(pdst=ip), timeout=1, verbose=False)
                        mac = res[0][1].hwsrc if res else "Unknown"
                        active_hosts.append({"ip": ip, "mac": mac})

            # Phase 3: Analysis & Service Fingerprinting
            table = Table(title=f"Elite Intel: {target}", border_style="bold red", expand=True)
            ...
            task3 = progress.add_task(f"[magenta]Analyzing {len(active_hosts)} live hosts...", total=len(active_hosts))
            ...
            for host in active_hosts:
                ...
                if pkt:
                    ...
                    if ttl <= 64: os_type = "Linux / IoT"
                    elif ttl <= 128: os_type = "Windows"
                    else: os_type = "Network Device"

                svc_info = []
                if do_deep:
                    with ThreadPoolExecutor(max_workers=10) as executor:
                        # Mapping service audit across top ports
                        futures = [executor.submit(self.service_audit, ip, p) for p in TOP_PORTS]
                        for f in as_completed(futures):
                            res = f.result()
                            if res: svc_info.append(res)

                table.add_row(
                    ip, 
                    f"{os_type}\n[dim]{vendor}[/dim]", 
                    "\n".join(svc_info) if svc_info else "[dim]None Detected[/dim]", 
                    mac
                )
                progress.update(task3, advance=1)

        console.print("\n", table)
        console.print(f"[bold green][+] Total Active Hosts Found: {len(active_hosts)}[/bold green]")
