...
class ScannerEngine:
    ...
    def os_fingerprint(self, ip):
        try:
            ...
            if sr1(IP(dst=ip)/ICMP(), timeout=0.8, verbose=0):
                return "Unknown (ICMP)"
        except:
            pass
        return "Unknown"
    ...
    def network_discovery(self):
        ...
        intensity = int(questionary.select("Intensity:", choices=[
                        "Stealth (10)", "Balanced (40)", "Aggressive (100)"], default="Balanced (40)", style=Q_STYLE).ask().split('(')[1].split(')')[0])
        do_deep = questionary.confirm(
            "Enable Service Fingerprinting?", default=False, style=Q_STYLE).ask()
        ...
        with Progress(SpinnerColumn(), TextColumn("{task.description}"), BarColumn(), TimeElapsedColumn(), console=console) as progress:
            ...
            task2 = progress.add_task(
                "[yellow]SYN Sweep...", total=len(ip_list))
            ...
            task3 = progress.add_task(
                f"[magenta]Analyzing...", total=len(active_hosts))
            for host in active_hosts:
                ...
                # SAVE TO DB
                db.log(
                    "Net-Mapper", ip, f"OS: {os_type} | Vendor: {vendor} | Services: {svc_str}", "INFO")
