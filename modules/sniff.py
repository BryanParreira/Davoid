...
class SnifferEngine:
    def __init__(self):
        self.max_entries    = 15
        self.pcap_filename  = (
            f"logs/capture_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pcap")
        # ── Per-instance state (no module-level globals that bleed between runs) ──
        self.session_pcap   = []
        self.captured_intel = []
    ...
    def parse_plaintext_creds(self, load, port):
        ...
        http_patterns  = [
            r"(?i)(user|username|login|email|pass|password|pwd|auth|token)"
            r"=([^&^ ^\r^\n]*)"]
        proto_patterns = [
            r"(?i)USER\s+(.*)\r\n",
            r"(?i)PASS\s+(.*)\r\n"]
        ...
        if port in [80, 8080]:
            for p in http_patterns:
                ...
                for match in matches:
                    intel.append(f"HTTP-DATA: {match[0]}={match[1]}")

        for p in proto_patterns:
            matches = re.findall(p, decoded)
            if matches:
                intel.append(f"AUTH-PROTO: {matches[0].strip()}")
    ...
    def callback(self, packet):
        ...
        # Limit PCAP size to avoid memory exhaustion on long captures
        if len(self.session_pcap) < 5000:
    ...
    def start(self):
        ...
        target_iface = questionary.select(
            "Select Interface:", choices=iface_list, style=Q_STYLE).ask()
        if not target_iface:
        ...
        console.print(
            ...
            "Saving critical intel to DB...")

        try:
            with Live(self.generate_table(),
                      refresh_per_second=2, screen=False) as live:
                def wrapped_cb(pkt):
                    self.callback(pkt)
                    live.update(self.generate_table())

                sniff(iface=target_iface,
                      prn=wrapped_cb,
                      store=0,
                      filter="ip")
...
def run_sniffer():
    # Instantiate fresh every time — no stale state from previous runs
    SnifferEngine().start()
